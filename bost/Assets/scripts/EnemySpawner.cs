...
public class EnemySpawner : ObjectPool<EnemySpawner, Enemy, Vector2>, IDataPersister
{
    ...
    void Start()
    {
        ...
        {
            Pop(transform.position + transform.right * Random.Range(-spawnArea * 0.5f, spawnArea * 0.5f)); //エリアを指定して生成
        }

        m_CurrentSpawnedEnemyCount = spawnCount; //生成数
        m_TotalSpawnedEnemyCount += concurrentEnemiesToBeSpawned; //一回当たりのスポーン数
...
public class Enemy : PoolObject<EnemySpawner, Enemy, Vector2>
{
    ...
    {
        damageable = instance.GetComponent<Damagerable>();
    ...
    protected void ReturnToPoolEvent(Damager dmgr, Damagerable dmgbl)
    {
