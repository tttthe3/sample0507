...
public class StoryFlag :ScriptableObject
{
    ...
    [SerializeField]
    private Init_KindofLoad[]  Init_Loadtype_mul; //シーン移動時にロードするか、イベント直後にロードするか、ステージ移動でロードするか  同じシーン内なら後者２つ、元からステージにあるタイプはロード不要
    ...
    [SerializeField]
    private Dictionary<string, Init_KindofLoad> Kindtoname;
    ...
    public Init_KindofLoad[] GetInit_KindofLoad_mul()
    {
        return Init_Loadtype_mul;
    }
